...
async function answerQuestion(web: WebClient, messageC: any, person: any = "", isEmail: boolean = false, isMention: boolean = false) {
    try {
        ...
        if (isEmail) {
            ...
            //    prompt
            //)
            responseOpenAI = "Subject: Increase 20% response rate in 20 seconds \n Content: \nHi Andrew, \n\nYour blog post on Dropbox’s product-led approach was really interesting - completely agree with your point that the job of the sales team is totally different when you have thousands of users.\n\nA lot of PLG focused growth managers (like you) find that personalized outreach messages based on specific user behavoir is much more effective at driving response rates.\n\nDropbox users in the enterprise space probably have very difference needs based on their company, product and buyer persona. I’m guessing you are sending a similar sequence to buyers at Airbnb as the Gitlab. Trust me, these folks want to hear very different things. \n\nCompanies like Box, Notion and Atlassian have found that AI-agents (like the ones from Waroom) can help drive a 15-20% improvement in response rate with highly personalized tactics and email campaigns.  \n\nAssuming Dropbox probably invests a lot in improving email response rate, and there’s a lot to do - can we make certain tasks faster or easier for you? \nWei\n-\n Wei Zhu\nCustomer Advocate | Cofounder \nWaroom.ai"
