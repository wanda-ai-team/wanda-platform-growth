...
const app = next({ dev, hostname: HOSTNAME, port: PORT }) // change here
...
app.prepare().then(() => {
    ...
    const serverF = server.listen(PORT, (err) => {
        if (err) throw err;
        console.log(`Server is running on port ${PORT}`);
    });

    // Handle the upgrade request
    serverF.on('upgrade', (request, socket, head) => {
