...
async function transcribeAudio(api_token: any, audio_url: any) {
    ...
    while (true) {
      ...
      const speakers = transcriptionResult.utterances.map((utterance: any) => {return {speaker: utterance.speaker, text: utterance.text}})
      const responseO = {speakers: speakers, text: transcriptionResult.text}
      return JSON.stringify(responseO)
